 ...
 *
 * @method     ChildClienteQuery leftJoinCategorias($relationAlias = null) Adds a LEFT JOIN clause to the query using the Categorias relation
 * @method     ChildClienteQuery rightJoinCategorias($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Categorias relation
 * @method     ChildClienteQuery innerJoinCategorias($relationAlias = null) Adds a INNER JOIN clause to the query using the Categorias relation
 *
...
abstract class ClienteQuery extends ModelCriteria
{
     ...
     *
     * @param \Categorias|ObjectCollection $categorias  the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
    ...
    public function filterByCategorias($categorias, $comparison = null)
    {
        if ($categorias instanceof \Categorias) {
            return $this
                ->addUsingAlias(ClienteTableMap::COL_ID, $categorias->getIdCliente(), $comparison);
        } elseif ($categorias instanceof ObjectCollection) {
            return $this
                ->useCategoriasQuery()
                ->filterByPrimaryKeys($categorias->getPrimaryKeys())
                ->endUse();
        } else {
