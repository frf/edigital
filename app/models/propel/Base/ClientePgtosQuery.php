 ...
 *
 * @method     ChildClientePgtosQuery leftJoinMoeda($relationAlias = null) Adds a LEFT JOIN clause to the query using the Moeda relation
 * @method     ChildClientePgtosQuery rightJoinMoeda($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Moeda relation
 * @method     ChildClientePgtosQuery innerJoinMoeda($relationAlias = null) Adds a INNER JOIN clause to the query using the Moeda relation
 *
 ...
 *
 * @method     \ClienteQuery|\MoedaQuery|\ProdutosQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 * @method     ChildClientePgtos requireOneByDtpagamento(string $dtpagamento) Return the first ChildClientePgtos filtered by the dtpagamento column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
...
abstract class ClientePgtosQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT id, valor, idproduto, idcliente, idmoeda, descricao, ispaid, nota, dtpagamento FROM cliente_pgtos WHERE id = :p0';
        try {
    ...
    public function filterByMoeda($moeda, $comparison = null)
    {
        if ($moeda instanceof \Moeda) {
            return $this
                ->addUsingAlias(ClientePgtosTableMap::COL_IDMOEDA, $moeda->getId(), $comparison);
            ...
            return $this
                ->addUsingAlias(ClientePgtosTableMap::COL_IDMOEDA, $moeda->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
